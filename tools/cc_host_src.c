...
typedef enum { TY_INT, TY_FLOAT, TY_CHAR, TY_VOID, TY_PTR, TY_ARRAY, TY_STRUCT } TypeKind;
...
enum {
    ...
    TOK_TYPEDEF, TOK_STRUCT, TOK_EXTERN, TOK_ENUM,
    TOK_INT, TOK_FLOAT, TOK_KW_CHAR, TOK_VOID, TOK_SIZEOF, TOK_UNSIGNED, TOK_SIGNED,
    TOK_DOT, TOK_LBRACK, TOK_RBRACK, TOK_COLON, TOK_ARROW, TOK_AMP, TOK_ELLIPSIS,
    TOK_OR, TOK_XOR, TOK_TILDE, TOK_MOD, TOK_SHL, TOK_SHR,
    TOK_LAND, TOK_LOR, TOK_LOGNOT
};
...
static Type type_int, type_float, type_char, type_void;
...
static void compute_line_col(int *out_line, int *out_col) {
    int line = 1;
    int col = 1;
    int i;
    for (i = 0; i < pos && src[i]; i++) {
        if (src[i] == '\n') { line++; col = 1; }
        else col++;
    }
    *out_line = line;
...
static void next_token(void) {
    ...
    tok.ival = 0;
    tok.fval = 0.0f;
    if (!c) { tok.kind = TOK_EOF; return; }

    if (is_alpha(c)) {
        int i = 0;
        while (is_alnum(src[pos]) && i < 63) tok.text[i++] = src[pos++];
        tok.text[i] = 0;
    ...
    pos++;
    switch (c) {
    case '(': tok.kind = TOK_LPAREN; return;
    case ')': tok.kind = TOK_RPAREN; return;
    case '{': tok.kind = TOK_LBRACE; return;
    case '}': tok.kind = TOK_RBRACE; return;
    case '[': tok.kind = TOK_LBRACK; return;
    case ']': tok.kind = TOK_RBRACK; return;
    case ';': tok.kind = TOK_SEMI; return;
    case ',': tok.kind = TOK_COMMA; return;
    case ':': tok.kind = TOK_COLON; return;
    case '.':
        if (src[pos] == '.' && src[pos+1] == '.') { pos += 2; tok.kind = TOK_ELLIPSIS; return; }
        tok.kind = TOK_DOT; return;
    case '+': tok.kind = TOK_PLUS; return;
    case '-':
        if (src[pos] == '>') { pos++; tok.kind = TOK_ARROW; return; }
        tok.kind = TOK_MINUS; return;
    case '*': tok.kind = TOK_MUL; return;
    case '/': tok.kind = TOK_DIV; return;
    case '%': tok.kind = TOK_MOD; return;
    case '&':
        if (src[pos] == '&') { pos++; tok.kind = TOK_LAND; return; }
        tok.kind = TOK_AMP; return;
    case '|':
        if (src[pos] == '|') { pos++; tok.kind = TOK_LOR; return; }
        tok.kind = TOK_OR; return;
    case '^': tok.kind = TOK_XOR; return;
    case '~': tok.kind = TOK_TILDE; return;
    case '=':
        if (src[pos]=='=') { pos++; tok.kind = TOK_EQ; }
        else tok.kind = TOK_ASSIGN;
        return;
    case '!':
        if (src[pos]=='=') { pos++; tok.kind = TOK_NE; return; }
        tok.kind = TOK_LOGNOT; return;
    case '<':
...
static Type *new_type(TypeKind kind, Type *base, int array_len, StructDef *sdef) {
    ...
    else if (kind == TY_STRUCT) { t->size = sdef->size; t->align = sdef->align; }
    return t;
}
...
static int is_numeric(Type *t) { return is_int(t) || is_float(t) || is_char(t); }
...
static Type *promote(Type *t) { return is_char(t) ? &type_int : t; }
...
static StructDef *add_struct(const char *name) {
    StructDef *s = 0;
    if (name && name[0]) {
        s = find_struct(name);
        if (s) return s;
...
static Sym *add_global(const char *name, Type *ty) {
...
enum { ND_NUM, ND_FNUM, ND_VAR, ND_BIN, ND_ASSIGN, ND_CALL, ND_CAST, ND_UNARY, ND_ADDR, ND_DEREF, ND_INDEX, ND_MEMBER, ND_VAARG, ND_STR };

static Node *parse_expr(void);

static Type *parse_struct_spec(void);
...
static Type *parse_type_spec(void) {
    int uns = 0;
    if (consume(TOK_UNSIGNED)) uns = 1;
    if (consume(TOK_SIGNED)) uns = 0;
    ...
    if (consume(TOK_STRUCT)) return parse_struct_spec();
    if (consume(TOK_ENUM)) {
...
static int is_type_start(void) {
    if (tok.kind == TOK_INT || tok.kind == TOK_FLOAT || tok.kind == TOK_KW_CHAR || tok.kind == TOK_VOID || tok.kind == TOK_STRUCT || tok.kind == TOK_UNSIGNED || tok.kind == TOK_SIGNED || tok.kind == TOK_ENUM) return 1;
...
static Type *infer_type(Node *n) {
    ...
    case ND_VAR: {
        ...
        if (!s) { error_name("unknown var", n->name); }
        return s->type;
    }
    case ND_ADDR: {
    ...
    case ND_MEMBER: {
        ...
        if (!is_struct(t)) { puts("member struct\n"); sys_exit(1); }
...
static Node *parse_postfix(void) {
    ...
    for (;;) {
        if (consume(TOK_LPAREN)) {
            if (n->kind != ND_VAR) { error_here("call name\n"); }
            Node *call = new_node(ND_CALL);
            strncpy(call->name, n->name, 31);
            call->argc = 0;
...
static Type *gen_call(Node *n) {
    ...
    /* syscall builtins */
    if (!strcmp(n->name, "sys_write")) { if (n->argc != 3) { puts("sys_write args\n"); sys_exit(1); } gen_expr(n->args[0]); gen_expr(n->args[1]); gen_expr(n->args[2]); return emit_syscall_builtin(0, 3); }
    if (!strcmp(n->name, "sys_read")) { if (n->argc != 3) { puts("sys_read args\n"); sys_exit(1); } gen_expr(n->args[0]); gen_expr(n->args[1]); gen_expr(n->args[2]); return emit_syscall_builtin(1, 3); }
    if (!strcmp(n->name, "sys_list")) { if (n->argc != 2) { puts("sys_list args\n"); sys_exit(1); } gen_expr(n->args[0]); gen_expr(n->args[1]); return emit_syscall_builtin(2, 2); }
    if (!strcmp(n->name, "sys_load")) { if (n->argc != 3) { puts("sys_load args\n"); sys_exit(1); } gen_expr(n->args[0]); gen_expr(n->args[1]); gen_expr(n->args[2]); return emit_syscall_builtin(3, 3); }
    if (!strcmp(n->name, "sys_save")) { if (n->argc != 3) { puts("sys_save args\n"); sys_exit(1); } gen_expr(n->args[0]); gen_expr(n->args[1]); gen_expr(n->args[2]); return emit_syscall_builtin(4, 3); }
    if (!strcmp(n->name, "sys_exec")) { if (n->argc != 1) { puts("sys_exec args\n"); sys_exit(1); } gen_expr(n->args[0]); return emit_syscall_builtin(5, 1); }
    if (!strcmp(n->name, "sys_exit")) { if (n->argc != 1) { puts("sys_exit args\n"); sys_exit(1); } gen_expr(n->args[0]); return emit_syscall_builtin(6, 1); }
    if (!strcmp(n->name, "sys_sbrk")) { if (n->argc != 1) { puts("sys_sbrk args\n"); sys_exit(1); } gen_expr(n->args[0]); return emit_syscall_builtin(7, 1); }
    if (!strcmp(n->name, "sys_rename")) { if (n->argc != 2) { puts("sys_rename args\n"); sys_exit(1); } gen_expr(n->args[0]); gen_expr(n->args[1]); return emit_syscall_builtin(8, 2); }
    if (!strcmp(n->name, "sys_delete")) { if (n->argc != 1) { puts("sys_delete args\n"); sys_exit(1); } gen_expr(n->args[0]); return emit_syscall_builtin(9, 1); }
    ...
    if (!strcmp(n->name, "sys_setcursor")) { if (n->argc != 2) { puts("sys_setcursor args\n"); sys_exit(1); } gen_expr(n->args[0]); gen_expr(n->args[1]); return emit_syscall_builtin(12, 2); }
    if (!strcmp(n->name, "sys_vmode")) { if (n->argc != 1) { puts("sys_vmode args\n"); sys_exit(1); } gen_expr(n->args[0]); return emit_syscall_builtin(13, 1); }
    if (!strcmp(n->name, "sys_blit")) { if (n->argc != 1) { puts("sys_blit args\n"); sys_exit(1); } gen_expr(n->args[0]); return emit_syscall_builtin(14, 1); }
    if (!strcmp(n->name, "sys_palette")) { if (n->argc != 2) { puts("sys_palette args\n"); sys_exit(1); } gen_expr(n->args[0]); gen_expr(n->args[1]); return emit_syscall_builtin(15, 2); }
    ...
    for (int i = n->argc - 1; i >= 0; i--) {
        ...
        if (is_struct(pt) || is_array(pt)) { puts("param struct/array\n"); sys_exit(1); }
    ...
    int pos = code_len;
    emit32(0);
    add_sym_fix(pos, RELOC_REL32, n->name);
...
static Type *gen_addr(Node *n) {
    ...
    case ND_MEMBER: {
        ...
        if (!is_struct(bt)) { puts("member struct\n"); sys_exit(1); }
...
static Type *gen_expr(Node *n) {
    ...
    case ND_VAR: {
        ...
        if (is_struct(s->type)) { puts("struct rvalue\n"); sys_exit(1); }
        emit_load_addr(s);
    ...
    case ND_DEREF: {
        Type *t = gen_addr(n);
        if (is_struct(t) || is_array(t)) { puts("struct rvalue\n"); sys_exit(1); }
    ...
    case ND_ASSIGN: {
        ...
        if (is_struct(lt) || is_array(lt)) { puts("assign struct/array\n"); sys_exit(1); }
    ...
    case ND_VAARG: {
        ...
        if (is_struct(n->type) || is_array(n->type)) { puts("va_arg type\n"); sys_exit(1); }
...
static void parse_decl_stmt(int expect_semi) {
    ...
    for (;;) {
        ...
        if (consume(TOK_ASSIGN)) {
            if (is_array(ty) || is_struct(ty)) { puts("init array/struct\n"); sys_exit(1); }
            Node *e = parse_expr();
            emit_load_addr(s);
            Type *t = gen_expr(e);
...
static void gen_return(Node *e) {
    ...
    if (is_float(current_ret)) {
        ...
        if (is_float(t)) conv_stack_to_int(0);
        emit_pop_eax();
    }
    emit_jmp_label(current_epilogue_label);
}
        ...
        if (!void_only) {
            if (consume(TOK_ELLIPSIS)) {
                ...
                do {
                    ...
                    if (is_struct(pty)) { error_here("param struct\n"); }
...
static void add_func_decl(Type *ret, const char *name, ParamInfo *pi) {
...
static void parse_function_def(Type *ret, const char *name, ParamInfo *pi) {
    Func *f = add_func(name, ret);
    f->defined = 1;
    f->ret = ret;
    f->param_count = pi->count;
...
static void parse_global(Type *ty, const char *name) {
    if (find_global(name)) { puts("dup global\n"); sys_exit(1); }
    Sym *s = add_global(name, ty);
    if (consume(TOK_ASSIGN)) {
        if (is_array(ty)) {
...
static void parse_program(void) {
    ...
    while (tok.kind != TOK_EOF) {
        ...
        int is_extern = 0;
        if (consume(TOK_EXTERN)) is_extern = 1;
        Type *base = parse_type_spec();
        if (!base) { error_here("type?\n"); }
        ...
        Type *ty = parse_declarator(base, name);
        if (tok.kind == TOK_LPAREN) {
            if (is_array(ty) || is_struct(ty)) { error_here("func return\n"); }
            ParamInfo pi = parse_param_list();
            if (tok.kind == TOK_SEMI) {
                next_token();
                add_func_decl(ty, name, &pi);
            } else {
                parse_function_def(ty, name, &pi);
            }
        } else {
            if (is_extern) {
...
static int pp_eval_if(Preproc *pp, const char *expr) {
    while (pp_is_space(*expr)) expr++;
    if (*expr == '!') return !pp_eval_if(pp, expr + 1);
    if (!strncmp(expr, "defined", 7)) {
        ...
        if (*expr == '(') {
            ...
            while (pp_is_alnum(*expr) && ni < 31) name[ni++] = *expr++;
            name[ni] = 0;
    ...
    int neg = 0;
    if (*expr == '-') { neg = 1; expr++; }
    while (*expr >= '0' && *expr <= '9') { v = v * 10 + (*expr - '0'); expr++; }
    return neg ? -v : v;
}
        ...
        if (*q == '#') {
            ...
            if (!strncmp(q, "include", 7) && pp_is_space(q[7])) {
                ...
                if (end) {
                    ...
                    char incbuf[65536];
                    int n = sys_load(name, incbuf, sizeof(incbuf) - 1);
...
static int compile_obj(const char *srcname, Obj *obj, int skip_stdio) {
    static char srcbuf[65536];
    static char ppbuf[131072];
    ...
    for (int i = 0; i < funcs_len; i++) {
        ...
        if (f->defined) {
            ...
            obj_add_sym(obj, f->name, SEC_TEXT, off, 0, 1, 1);
        } else {
            obj_add_sym(obj, f->name, 0, 0, 0, 0, 1);
        }
    }
    for (int i = 0; i < globals_len; i++) {
        Sym *s = &globals[i];
        if (s->is_extern) {
            obj_add_sym(obj, s->name, 0, 0, 0, 0, 0);
        } else {
            int sz = type_size(s->type);
            obj_add_sym(obj, s->name, SEC_DATA, s->offset, sz, 1, 0);
        }
    }
    ...
    for (int i = 0; i < addr_fix_len; i++) {
        ...
        if (addr_fix[i].kind == AF_STR) {
            ...
            r->type = RELOC_ABS32;
            r->addend = 0;
            strncpy(r->name, globals[addr_fix[i].index].name, 31);
        }
    }
...
static int link_objects(Obj *objs, int obj_count,
    ...
    for (int i = 0; i < sym_len; i++) {
        if (!syms[i].defined) { puts("undef "); puts(syms[i].name); puts("\n"); return -1; }
    }
